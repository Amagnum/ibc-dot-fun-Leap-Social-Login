...
import { capsuleState, showCapsuleModelState } from '@/leap-cosmos-capsule/atoms'
...
export default function useCapsule() {
  ...
  const [percentKeygenDone, setPercentKeygenDone] = useState(paillierGenDone ? 25 : 0)

  useEffect(() => {
    if (
      !showCapsuleModal &&
      [ModalStep.LOGIN_DONE, ModalStep.ACCOUNT_CREATION_DONE].includes(currentStep)
    ) {
  ...
  useEffect(() => {
    ...
    async function genWallet() {
      ...
      const createWalletRes = await capsule.createWallet(true, keygenStatusFunction)
